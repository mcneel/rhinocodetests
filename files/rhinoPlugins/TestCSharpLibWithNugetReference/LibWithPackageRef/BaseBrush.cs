...
namespace BrushTools
{
    ...
    { 
        ...
        protected double m_maxValue = 60f;
        protected double m_minValue = 1f;
        protected bool _mouseDown = false;
        protected Easing.Ease m_ease;

        public BaseBrush(double size = 8f, double falloff = 16f, double strength = 0.1f)
        {
            m_size = size;
            m_falloff = falloff;
            m_strength = strength;
            ...
            var optsizedouble = new OptionDouble(m_size, 1, 100);
            var optfalloffdouble = new OptionDouble(m_falloff, 1, 100);
            var optstrengthdouble = new OptionDouble(m_strength, 1, 10);
        ...
        protected double ComputeIncrement(double dist, double start, bool reverse)
        {
            double value = start;
            double incr = m_strength * (dist < m_size ? 1.0d : m_ease.Out(Convert.ToSingle(dist)));
